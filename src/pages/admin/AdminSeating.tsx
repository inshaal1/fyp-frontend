...
import type { SeatingHall, SimpleStudent, Seat } from "@/services/types";
...
export default function AdminSeating() {
  ...
  const [halls, setHalls] = useState<SeatingHall[]>([]);
  ...
  useEffect(() => {
    api.getSeatingHalls().then((h) => {
      setHalls(h);
      if (h.length > 0) setSelectedHallId(h[0].id);
    });
    api.getSeatingStudents().then(setStudents);
  }, []);

  const selectedHall = halls.find((h) => h.id === selectedHallId) || halls[0];

  const handleSeatingPlanUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const newHalls = await api.uploadSeatingPlan(file);
    setHalls(newHalls);
    setSelectedHallId(newHalls[0].id);
  ...
  const autoMapStudents = (studentList: SimpleStudent[]) => {
    setHalls((prev) => {
      let studentIndex = 0;
  ...
  const filteredStudents = students.filter(
    (s) => s.name.toLowerCase().includes(studentSearch.toLowerCase()) || s.id.toLowerCase().includes(studentSearch.toLowerCase())
  );
        ...
        <div className="flex items-center justify-center min-h-[60vh] animate-fade-in">
          <div className="text-center space-y-6 max-w-md">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">Upload a seating plan file (CSV/Excel) to define exam halls and seat layouts</p>
            </div>
            ...
            <Button onClick={() => seatingPlanInputRef.current?.click()} size="lg">
              <Upload className="h-4 w-4 mr-2" />Upload Seating Plan
            </Button>
          </div>
        </div>
      </DashboardLayout>
        ...
        <div className="flex items-center justify-center min-h-[60vh] animate-fade-in">
          <div className="text-center space-y-6 max-w-md">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">Upload a students file (CSV/Excel) to automatically map students to seats</p>
            </div>
            ...
            <Button onClick={() => studentsInputRef.current?.click()} size="lg">
              <Upload className="h-4 w-4 mr-2" />Upload Students List
            </Button>
          </div>
        </div>
      </DashboardLayout>
      ...
      <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3">
              ...
              <Button variant="outline" size="sm" className="text-xs sm:text-sm" onClick={() => studentsInputRef.current?.click()}>
                <FileSpreadsheet className="h-4 w-4 sm:mr-2" /><span className="hidden sm:inline">Upload Student Excel File</span>
              </Button>
              <Button variant="outline" size="sm" className="text-xs sm:text-sm" onClick={() => seatingPlanInputRef.current?.click()}>
                <Upload className="h-4 w-4 sm:mr-2" /><span className="hidden sm:inline">Upload New Plan</span>
              </Button>
              <Button variant="outline" size="sm" className="text-xs sm:text-sm" onClick={handleReset}>
                <RotateCcw className="h-4 w-4 sm:mr-2" /><span className="hidden sm:inline">Reset Layout</span>
              </Button>
              <Button size="sm" className="text-xs sm:text-sm" onClick={handleSave}>
                <Save className="h-4 w-4 sm:mr-2" /><span className="hidden sm:inline">Save Seating Plan</span>
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 sm:gap-4 text-xs sm:text-sm">
          <div className="px-3 sm:px-4 py-2 bg-card rounded-lg border border-border">
            <span className="text-muted-foreground">Total: </span><span className="font-semibold">{totalSeats}</span>
          </div>
          <div className="px-3 sm:px-4 py-2 bg-success/10 rounded-lg border border-success/20">
            <span className="text-muted-foreground">Assigned: </span><span className="font-semibold text-success">{assignedCount}</span>
          </div>
          <div className="px-3 sm:px-4 py-2 bg-muted rounded-lg border border-border">
            <span className="text-muted-foreground">Available: </span><span className="font-semibold">{totalSeats - assignedCount}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6">
          <div className="lg:col-span-3 bg-card rounded-lg border border-border p-4 sm:p-6 shadow-card overflow-hidden">
            <div className="mb-4 text-center">
              <div className="inline-block px-4 sm:px-6 py-1 bg-muted rounded text-xs font-medium text-muted-foreground uppercase tracking-wider">Front / Stage</div>
            </div>
                ...
                <div key={rowIndex} className="flex gap-1 sm:gap-2">
                  ...
                  {row.map((seat, colIndex) => (
                    <div
                      key={seat.id}
                      onClick={() => {
                        if (seat.studentId) handleClearSeat(rowIndex, colIndex);
                        else setSelectedSeat({ row: rowIndex, col: colIndex });
                      }}
            ...
            {selectedSeat ? (
              <div className="space-y-4">
                <div className="p-3 bg-primary/5 rounded-lg border border-primary/20">
                  ...
                  <p className="font-semibold text-lg text-primary">{selectedHall?.seats[selectedSeat.row][selectedSeat.col].id}</p>
                </div>
                <div className="relative">
                  <Input placeholder="Search student..." value={studentSearch} onChange={(e) => setStudentSearch(e.target.value)} className="pr-8" />
                  {studentSearch && (
                    <button onClick={() => setStudentSearch("")} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                      <X className="h-4 w-4" />
                    </button>
                  )}
                </div>
                <div className="max-h-64 overflow-y-auto space-y-1">
                  {filteredStudents.map((student) => (
                    <button key={student.id} onClick={() => handleAssignStudent(student.id, student.name)} className="w-full p-2 text-left rounded-lg hover:bg-muted transition-colors border border-transparent hover:border-border">
