...
import type { StudentListItem } from "@/services/types";
...
export default function InvigilatorStudents() {
  ...
  useEffect(() => {
    api.getStudentList().then(setStudents);
  }, []);
  ...
  const columns = [
    { header: "Student ID", accessor: "studentId" as keyof StudentListItem },
    { header: "Name", accessor: "name" as keyof StudentListItem },
    { header: "Seat Number", accessor: "seatNumber" as keyof StudentListItem },
    { header: "Exam Hall", accessor: "examHall" as keyof StudentListItem },
    {
      header: "Status",
      accessor: (row: StudentListItem) => (
  ...
  return (
    ...
    >
      <div className="space-y-4 sm:space-y-6 animate-fade-in">
        <div className="flex flex-col gap-3 sm:gap-4">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            ...
            <Select value={hallFilter} onValueChange={setHallFilter}>
              <SelectTrigger className="w-full sm:w-36"><SelectValue placeholder="Hall" /></SelectTrigger>
              <SelectContent>
            ...
            {(statusFilter !== "all" || hallFilter !== "all" || searchQuery) && (
              <Button variant="ghost" size="icon" onClick={() => { setSearchQuery(""); setStatusFilter("all"); setHallFilter("all"); }}>
                <X className="h-4 w-4" />
              </Button>
            )}
          </div>
        ...
        <DataTable<StudentListItem> columns={columns} data={filteredStudents} onRowClick={(row) => setSelectedStudent(row)} />
              ...
              <div className="mt-6 space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <StatusBadge variant={selectedStudent.status === "Flagged" ? "destructive" : "success"} className="mt-1">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-muted/50 rounded-lg">
                      ...
                      <p className="font-medium flex items-center gap-1"><MapPin className="h-4 w-4" />{selectedStudent.seatNumber}</p>
                    </div>
                    <div className="p-3 bg-muted/50 rounded-lg">
                  ...
                  <div className={`p-4 rounded-lg border ${selectedStudent.alertCount > 0 ? "bg-destructive/5 border-destructive/20" : "bg-success/5 border-success/20"}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">{selectedStudent.alertCount > 0 ? `${selectedStudent.alertCount} Alert(s) Recorded` : "No Alerts"}</p>
                        <p className="text-sm text-muted-foreground">{selectedStudent.alertCount > 0 ? "Review in Alerts section" : "Student behavior is normal"}</p>
                      </div>
                    </div>
                  </div>
                </div>
