...
const MOCK_USERS: Record<string, { password: string; role: "invigilator" | "admin"; name: string }> = {
...
export async function login(universityId: string, password: string): Promise<User | null> {
  await delay(800);
...
export async function getInvigilatorDashboardStats(): Promise<DashboardStats> {
  await delay();
  return { totalStudents: 248, activeAlerts: 12, examHalls: 6 };
}

export async function getExamHallOptions(): Promise<ExamHallOption[]> {
  await delay();
  return [
...
export async function updateAlertStatus(alertId: string, newStatus: "Reviewed" | "Ignored"): Promise<void> {
  await delay();
...
const mockStudentList: StudentListItem[] = [
...
export async function getExamHalls(): Promise<ExamHall[]> {
  await delay();
...
const adminExamHalls: AdminExamHall[] = [
...
export async function getAdminExamHalls(): Promise<AdminExamHall[]> {
  await delay();
  return [...adminExamHalls];
}

export async function createAdminExamHall(hall: Omit<AdminExamHall, "id">): Promise<AdminExamHall> {
  await delay();
...
export async function updateAdminExamHall(id: string, data: Partial<AdminExamHall>): Promise<void> {
  await delay();
  const index = adminExamHalls.findIndex((h) => h.id === id);
  if (index !== -1) Object.assign(adminExamHalls[index], data);
}

export async function deleteAdminExamHall(id: string): Promise<void> {
  await delay();
  const index = adminExamHalls.findIndex((h) => h.id === id);
...
const seatingStudents: SimpleStudent[] = [
...
const seatingHalls: SeatingHall[] = [
...
export async function getSeatingHalls(): Promise<SeatingHall[]> {
  await delay();
  return JSON.parse(JSON.stringify(seatingHalls));
}

export async function getSeatingStudents(): Promise<SimpleStudent[]> {
  await delay();
  return [...seatingStudents];
}

export async function saveSeatingPlan(halls: SeatingHall[]): Promise<void> {
  await delay();
  // In real API, persist to database
}

export async function uploadSeatingPlan(_file: File): Promise<SeatingHall[]> {
  await delay();
  return JSON.parse(JSON.stringify(seatingHalls));
}

export async function uploadStudentsList(_file: File): Promise<SimpleStudent[]> {
  await delay();
  return [...seatingStudents];
}
...
export async function exportReport(_format: "pdf" | "csv"): Promise<void> {
  await delay();
